...
Hardware: NVIDIA GPU (RTX, V100, A100, H100) recommended. CPU mode is available but significantly slower.

Drivers: CUDA toolkit compatible with your GPU.
...
cd HEXA_TopOpt
Instantiate the environment (this installs required packages like CUDA.jl, LinearAlgebra, YAML, etc.):
...
How it Works
Initialization: Run.jl verifies dependencies and checks for a GPU.

Hardware Profiling: It runs a stress test on the GPU to generate configs/_machine_limits.jl, determining the maximum safe element count to prevent Out-Of-Memory (OOM) crashes.

Batch Processing: It reads the job queue and sequentially executes simulations.
...
*_solution.vti: Full 3D volumetric data (Density, Stress, Displacement) viewable in ParaView.
